...
export const story = {
  ...
  whereIStand: [
    ...
    "거기서 본 것은 기술이 아니라 방향이었습니다. 기업이 AI에 기대하는 것은 '자동화'가 아니라 '검증 가능한 판단 보조'로 옮겨가고 있었고, 경영진용 AI가 CEO가 아니라 CFO부터 시작되는 이유는 신뢰였습니다. AI 도입의 병목은 성능이 아니라 검증 가능성이고, AX의 승부처는 모델이 아니라 문제 정의 → 기획 → 구현의 정합성이라는 것. 심사위원의 \"비즈니스는 훌륭하나 기술이 부족하다, 우리는 둘 다 되는 사람을 원한다\"는 피드백까지 포함해, 대기업들이 지금 고민하고 있는 것과 시대가 무엇을 원하는지 알게 된 계기였습니다.",
  ],
  experience: [
