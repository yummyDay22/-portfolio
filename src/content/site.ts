...
export const story = {
  heroLead: "문제를 정의하고, 기획하고, 직접 만들어 검증합니다",
  heroRest: "AI가 만든 결과물이라도 노트 없이 30초 안에 설명하지 못하면 제 것이 아니라고 생각합니다.",
  howIGotHere: [
    "한 번에 대박을 노리기보다 반복 가능한 사이클을 갖추는 데 집중해 왔습니다. 기회 감지 → 현장 시장 조사 → AI로 풀 지점 설계 → MVP 플래닝 → 작게 시행 → 가설이 맞으면 확대. 도메인을 바꿔 가며 이 사이클을 여러 번 돌렸고, 그중 가장 멀리 끌고 간 케이스가 수리논술 AI 첨삭 플랫폼 수리넷입니다.",
    "어떤 문제든 같은 순서로 풉니다. 주어진 자료를 전부 읽고 마스터 문제와 파생 문제를 정리한 뒤, AI와 발산·수렴을 반복해 계획을 하나로 모으고, 구현은 AI가 만들고 제가 검토합니다. 이 방식으로 도메인 지식이 전무한 부동산 예측 해커톤에서 우승했고, 3주 만에 사내 RAG 챗봇을 배포했습니다.",
  ],
  whereIStand: [
